 ...
 * modification for SPI based SSD1306 OLED Displays: S Vignesh<s.vignesh06101999@gmail.com>
   ----------------------------------------------------------------------
   	Copyright (C) S Vignesh, 2024
...
/* Write data */
#define SSD1306_WRITEDATA(data)            ssd1306_SPI_Write_Data((data))
/* Absolute value */
...
void SSD1306_UpdateScreen(void) {
	...
	for (m = 0; m < 8; m++) {
		...
		/* Write multi data */
		ssd1306_SPI_WriteMulti(SSD1306_I2C_ADDR, 0x40, &SSD1306_Buffer[SSD1306_WIDTH * m], SSD1306_WIDTH);
	}
}
...
void ssd1306_SPI_WriteMulti(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {
uint8_t dt[256];

uint8_t i;
HAL_GPIO_WritePin(GPIOB,GPIO_PIN_0,GPIO_PIN_RESET); //CS
HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_SET); //D/C
for(i = 0; i < count; i++){
dt[i] = data[i];
HAL_SPI_Transmit(&hspi1, (uint8_t *) &dt[i], 1, 10);

}
HAL_GPIO_WritePin(GPIOB,GPIO_PIN_0,GPIO_PIN_SET); //CS
HAL_GPIO_WritePin(GPIOB,GPIO_PIN_1,GPIO_PIN_RESET); //D/C
}
